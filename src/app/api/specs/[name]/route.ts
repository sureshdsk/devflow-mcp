...
export async function GET(
  ...
  try {
    ...
    const tasksApproved = spec.statuses.find((s) => s.id === "tasks")?.state === "done";
    let developmentState: "blocked" | "ready" | "in_progress" | "done" = "blocked";
    let tasks: Array<{ id: string; title: string; status: string; assignedAgent: string | null; priority: string }> = [];
    ...
    if (tasksApproved) {
      ...
      if (tasks.length === 0) {
        developmentState = "ready"; // tasks approved but not yet promoted
